...
class Builder {
    ...
    populateTools(rootFile, buildTools) {
        ...
        buildTools.forEach(tool => {
            ...
            if (tool.args) {
                tool.args = tool.args.map((0, utils_1.replaceArgumentPlaceholders)(rootFile, this.tmpDir));
            }
            if (tool.env) {
                Object.keys(tool.env).forEach(v => {
                    const e = tool.env && tool.env[v];
                    if (tool.env && e) {
                        tool.env[v] = (0, utils_1.replaceArgumentPlaceholders)(rootFile, this.tmpDir)(e);
                    }
                });
            }
